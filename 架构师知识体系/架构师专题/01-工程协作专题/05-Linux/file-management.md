...
#### `find`：实时查找文件/目录
```shell
find . -name "file.txt"           # 在当前目录下，查找 file.txt
find /home -name "*.log"          # 在 /home 下，查找所有 .log 文件（及其子目录）
find / -type f -size +1G          # 在 /根目录下，查找大于 1GB 的文件
find . -type d -mtime -7          # 在当前目录下，查找 7 天内修改过的目录
```
## 文件所有者与所属组
...
- 改变权限与所有权：
  - `chmod`、`chown`、`chgrp`
- 默认权限掩码 `umask`
- 高级权限位：setuid、setgid、sticky bit
- 文件属性工具：
  - `chattr`、`lsattr`
