...
# 显示未暂存的更改：（🧩 工作区 vs 📦 暂存区）
git diff
# 显示所有未提交的更改：（🧩 工作区【含暂存】 vs HEAD）
git diff HEAD
# 仅显示已暂存（添加过但未提交）的更改：(📦 暂存区 vs 最新提交（HEAD）)
git diff --cached
git diff --staged
...
$ git branch - testing
```
### `git checkout`：切换分支 / 文件检出
不带 `--` 的 `git checkout` 是切换分支；带 `--` 表示是还原文件。
```shell
# 创建并切换到新分支
...
git extract <branch-or-commit> <file>   # 别名简化
```
### `git switch`：分支切换（推荐）
...
手动编辑冲突文件，清除冲突标记 `<<<<<<<` `=======` `>>>>>>>`。 
### `git rebase`：变基
用于整理提交历史，把分支“平铺”到目标分支之上。
```shell
git switch feature
git rebase main
```
⚠️ 不要对**已推送分支**使用 rebase！
